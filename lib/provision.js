...
// In-memory job log store so the UI can poll long-running installs.
const g = globalThis;
if (!g.__PAL_JOBS) g.__PAL_JOBS = new Map();
const JOBS = g.__PAL_JOBS;

function newJob() {
  const id = crypto.randomUUID();
  JOBS.set(id, { id, lines: [], done: false, ok: null, worldId: null });
  return id;
}
function jobLog(id, line) {
  const j = JOBS.get(id);
  if (j) { j.lines.push(line); if (j.lines.length > 2000) j.lines.shift(); }
}
...
async function provisionWorld(jobId, worldId) {
  ...
  try {
    ...
    const code = await steam.installOrUpdate(world.install_dir, log);
    if (code !== 0) throw new Error(`SteamCMD failed (code ${code})`);

    // capture build id
    const bid = steam.readInstalledBuildId(world.install_dir);
    ...
    finishJob(jobId, true, worldId);
  } catch (e) {
    log(`ERROR: ${e.message}`);
    dbm.updateWorld(worldId, { status: "stopped" });
    finishJob(jobId, false, worldId);
  }
}

function finishJob(id, ok, worldId) {
  const j = JOBS.get(id);
  if (j) { j.done = true; j.ok = ok; j.worldId = worldId; }
}
