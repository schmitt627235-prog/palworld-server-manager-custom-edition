...
function migrate(d) {
  d.exec(`
    ...
    CREATE TABLE IF NOT EXISTS reserved_slot_settings (
      world_id TEXT PRIMARY KEY,
      enabled INTEGER NOT NULL DEFAULT 0,
      reserved_slots INTEGER NOT NULL DEFAULT 1,
      message TEXT NOT NULL DEFAULT 'Der verbleibende Serverplatz ist reserviert.',
      updated_at INTEGER NOT NULL
    );

    CREATE TABLE IF NOT EXISTS reserved_players (
      world_id TEXT NOT NULL,
  ...
  if (!cols.includes("playit_enabled")) {
    d.exec("ALTER TABLE worlds ADD COLUMN playit_enabled INTEGER NOT NULL DEFAULT 0");
  }
  if (!cols.includes("playit_public_ip")) {
    d.exec("ALTER TABLE worlds ADD COLUMN playit_public_ip TEXT NOT NULL DEFAULT ''");
  }
  if (!cols.includes("playit_public_port")) {
    d.exec("ALTER TABLE worlds ADD COLUMN playit_public_port INTEGER NOT NULL DEFAULT 14815");
  }
  ...
  if (!cols.includes("discord_webhooks")) {
    d.exec("ALTER TABLE worlds ADD COLUMN discord_webhooks TEXT");
    // One-time backfill for people upgrading who were already using Discord: turn
    // their single webhook into one "Default Channel" and route every event to it,
    ...
    try {
      const rows = d.prepare(
        "SELECT world_id, discord_webhook, discord_relay_chat FROM worlds WHERE discord_webhook IS NOT NULL AND discord_webhook <> ''"
      ).all();
      for (const r of rows) {
        const url = String(r.discord_webhook || "").trim();
        if (!url) continue;
        const routes = {};
        for (const k of ["start", "stop", "restart", "crash", "backup", "update"]) routes[k] = "default";
  ...
  if (!cols.includes("server_password")) {
    d.exec("ALTER TABLE worlds ADD COLUMN server_password TEXT NOT NULL DEFAULT ''");
  }
  ...
  if (!cols.includes("warn_enabled")) {
    d.exec("ALTER TABLE worlds ADD COLUMN warn_enabled INTEGER NOT NULL DEFAULT 0");
  }
  if (!cols.includes("warn_lead_minutes")) {
    d.exec("ALTER TABLE worlds ADD COLUMN warn_lead_minutes INTEGER NOT NULL DEFAULT 10");
  }
  if (!cols.includes("warn_interval_minutes")) {
    d.exec("ALTER TABLE worlds ADD COLUMN warn_interval_minutes INTEGER NOT NULL DEFAULT 2");
  }
  if (!cols.includes("warn_message")) {
    d.exec("ALTER TABLE worlds ADD COLUMN warn_message TEXT NOT NULL DEFAULT 'The server will restart in {minutes} minute(s). Please get to a safe place.'");
  }
  ...
  if (!bcols.includes("status")) {
    d.exec("ALTER TABLE broadcasts ADD COLUMN status TEXT NOT NULL DEFAULT 'pending'");
  }
  ...
  if (!scols.includes("interval_minutes")) d.exec("ALTER TABLE schedules ADD COLUMN interval_minutes REAL");
  if (!scols.includes("message")) d.exec("ALTER TABLE schedules ADD COLUMN message TEXT");
  if (!scols.includes("join_match")) d.exec("ALTER TABLE schedules ADD COLUMN join_match TEXT");
}
...
function updateWorld(id, patch) {
  ...
  if (merged.playit_enabled === undefined) merged.playit_enabled = 0;
  if (merged.playit_public_ip === undefined || merged.playit_public_ip === null) merged.playit_public_ip = "";
  if (merged.playit_public_port === undefined || merged.playit_public_port === null) merged.playit_public_port = 14815;
  ...
  db().prepare(`UPDATE worlds SET
    ...
    community_server=@community_server, playit_enabled=@playit_enabled,
    playit_public_ip=@playit_public_ip, playit_public_port=@playit_public_port,
...
function deleteWorld(id) {
  ...
  db().prepare("DELETE FROM reserved_players WHERE world_id=?").run(id);
  db().prepare("DELETE FROM reserved_slot_settings WHERE world_id=?").run(id);
}
...
function insertSchedule(s) {
  db().prepare(`INSERT INTO schedules(id,world_id,job_type,mode,interval_hours,interval_minutes,time_of_day,message,join_match,enabled,created_at)
    VALUES(@id,@world_id,@job_type,@mode,@interval_hours,@interval_minutes,@time_of_day,@message,@join_match,@enabled,@created_at)`).run(s);
}
...
// ---- reserved slots (Community Edition 3.1.1) ----
function getReservedSlots(worldId) {
  const settings = db().prepare("SELECT * FROM reserved_slot_settings WHERE world_id=?").get(worldId) || {
    world_id: worldId, enabled: 0, reserved_slots: 1,
    message: "Der verbleibende Serverplatz ist reserviert.", updated_at: 0,
  };
  const players = db().prepare(
    "SELECT * FROM reserved_players WHERE world_id=? ORDER BY role ASC, display_name ASC, steam_id ASC"
  ).all(worldId);
  return { settings, players };
}
function saveReservedSlots(worldId, settings) {
  db().prepare(`INSERT INTO reserved_slot_settings(world_id,enabled,reserved_slots,message,updated_at)
    VALUES(?,?,?,?,?) ON CONFLICT(world_id) DO UPDATE SET
    enabled=excluded.enabled,reserved_slots=excluded.reserved_slots,message=excluded.message,updated_at=excluded.updated_at`)
    .run(worldId, settings.enabled ? 1 : 0, settings.reserved_slots, settings.message, Date.now());
  return getReservedSlots(worldId);
}
function upsertReservedPlayer(worldId, player) {
  db().prepare(`INSERT INTO reserved_players(world_id,steam_id,display_name,role,note,enabled,created_at)
    VALUES(?,?,?,?,?,?,?) ON CONFLICT(world_id,steam_id) DO UPDATE SET
    display_name=excluded.display_name,role=excluded.role,note=excluded.note,enabled=excluded.enabled`)
    .run(worldId, player.steam_id, player.display_name || "", player.role || "vip", player.note || "", player.enabled === false ? 0 : 1, Date.now());
  return getReservedSlots(worldId);
}
function deleteReservedPlayer(worldId, steamId) {
  db().prepare("DELETE FROM reserved_players WHERE world_id=? AND steam_id=?").run(worldId, steamId);
  return getReservedSlots(worldId);
}
...
module.exports = {
  ...
  getReservedSlots, saveReservedSlots, upsertReservedPlayer, deleteReservedPlayer,
