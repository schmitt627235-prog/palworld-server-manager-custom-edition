...
function recordChat(worldId, chat) {
  ...
  relayChatToDiscord(entry);
}

// Post a captured chat message to the configured Discord webhook, if the user has
// enabled chat relay. Uses the player's name as the webhook username so it reads like
// a cross-chat feed. Fire-and-forget; never throws into the tailer.
function relayChatToDiscord(entry) {
  try {
    if (!dbm.getSetting("discordRelayChat", false)) return;
...
// The bundled mod's install location inside a server.
function chatModDir(installDir) {
  return path.join(installDir, "Pal", "Binaries", "Win64", "Mods", "PSMChatRelay");
}
function chatModInstalled(installDir) {
  try { return fs.existsSync(path.join(chatModDir(installDir), "Scripts", "main.lua")); }
  catch { return false; }
}
...
function parseChatEntry(line) {
  if (line[0] === "{") {
    try {
      ...
      if (o && o.message) {
        return { name: o.name || "Player", message: String(o.message), channel: o.channel || null, at: o.at || Date.now() };
      }
    } catch {}
  }
...
module.exports = {
  ...
  chatModDir, chatModInstalled, chatFilePath, installChatMod, bundledChatModDir,
};
